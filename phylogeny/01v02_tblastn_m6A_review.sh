...
GENOME_DIR="/home/users/tskalicky/CEITEC/genomes/Danio_rerio"
...
echo "There are $length2 Athali blast databses that will be used for tblastn."
...
declare -a ATHALI_PROTEINS=($(find . -maxdepth 1 -name  '*.fa' -exec basename {} \; | sort -n))
...
# get length of an array
quantity="${#ATHALI_PROTEINS[@]}"
echo "There are $quantity Athali protein samples that will be blasted."
echo "Sample names are: ${ATHALI_PROTEINS[@]}"
#
for (( w=0;w<${quantity};w +=1 )); do
	SAMPLE="${ATHALI_PROTEINS[$w]}"
	SAMPLENAME=${SAMPLE%.*}
	if [[ -f $SAMPLE ]]; then
		...
		-out "tblastn_m6A_"$SAMPLENAME"_"${BLAST_DB[1]}"_search" -evalue 1e-20 \
		...
		-out "tblastn_m6A_"$SAMPLENAME"_"${BLAST_DB[2]}"_search" -evalue 1e-20 \
		-outfmt '6 qseqid sallseqid length pident mismatch gaps qstart qend sstart send evalue bitscore qseq sseq' \
		-max_target_seqs 1000000 -culling_limit 1 -num_threads $THREADS
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
